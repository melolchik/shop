...
public class PermissionManager {
    ...
    public static final int REQUEST_CODE_ACCESS_CAMERA = 505;
    ...
    public static String[] getPermissionListByCode(int requestCode){
        ...
        switch (requestCode){
            ...
            case REQUEST_CODE_ACCESS_BLUETOOTH:
                ...
                break;
            case REQUEST_CODE_ACCESS_CAMERA:
    ...
    public boolean isCameraPermissionGranted(Activity activity) {
        mActivity = activity;
        String permissionList[] = getPermissionListByCode(REQUEST_CODE_ACCESS_CAMERA);
...
//        log("checkPermissionGranted ");
   ...
   /*protected void onPermissionsCheckGranted(int codeRequest, List<String> notGrantedList) {
        if (notGrantedList == null || notGrantedList.isEmpty()) {
            onPermissionsGranted(codeRequest, true, null);
            return;
        } else
            onPermissionsGranted(codeRequest, false);
    }
       ...
       /* Manifest.permission.READ_EXTERNAL_STORAGE,
        ...
        Manifest.permission.ACCESS_COARSE_LOCATION
        boolean isExternalStorageGranted = true;
        boolean isLocationGranted = true;

        for (String permission : notGrantedList) {
            if (permission.equals(Manifest.permission.READ_EXTERNAL_STORAGE) ||
                    permission.equals(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                isExternalStorageGranted = false;
            }
       ...
       String permossions = "";
        String message = "";
        if (!isExternalStorageGranted && !isLocationGranted) {
            //show message about two permission
            permossions = getString(R.string.permissions_external_storage_and_location);
            message = getString(R.string.alert_about_many_permissions, permossions);
        }
        if (!isExternalStorageGranted && isLocationGranted) {
            //show message about External Storage permission
            permossions = getString(R.string.permissions_external_storage);
            message = getString(R.string.alert_about_one_permission, permossions);
        }
        if (!isLocationGranted && isExternalStorageGranted) {
            //show message about location permission
            permossions = getString(R.string.permissions_location);
            message = getString(R.string.alert_about_one_permission, permossions);
        }
        showAlertOk(message);
